...
int main() {
	...
	while (1) {
		...
		// 아래 구문을 통해서 데이터를 1,2,3 이렇게 저장된 방식이 -> 3,2,1 이렇게 저장된다.
		if (head == NULL) {			// 노드가 비어져 있다면
			head = newNode;			// 헤드가 새 노드의 주소를 가리키게 한다.
			tail = newNode;			// 꼬리도 새 노드의 주소를 가리키게 한다.
		}			
		else {			// 노드에 데이터가 들어 있다면
			newNode->next = head;			// 노드의 주소부분이 헤드를 가리키게 한다.
			head = newNode;			// 새 노드의 주소를 다시 헤드부분에 넣어 준다.
			// 위와 같이 작성하게 되면 새 노드는 항상 head를 가리키고 먼저 들어온 노드는 새 노드를 가리키게 된다.
			// 스택처럼 나중에 저장된 값부터 출려되고 먼저 저장된 값이 제일 나중에 출력되는 것이다.
		}
	}
	printf("\n");
